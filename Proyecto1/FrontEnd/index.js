((Utils) => {
  const App = {
    ...
    handlers: {
      pokemonFinderFormOnSubmit: async (e) => {
        ...
        try {
          ...
          if (document.miFormulario.evolutions.checked) {
    ...
    templates: {
      general: ({ id, sprites, name, weight, height, abilities, date }) => {
        console.log(date);
      ...
      encounters: ({ encounters }) => {
        ...
        encounters.map((element) => {
          table.push(`<h3>${element.location_area.name}</h3>`);
          element.version_details.map((index) => {
            console.log("---", index.version.name);
            table.push(`<p>Version: ${index.version.name}</p>`);
            index.encounter_details.map((indexs) => {
              table.push(
                `<li>${indexs.method.name} (${indexs.chance})</li>`
              );
            });
          });
        });
          ...
          encountersList.push(
            ` <div class="cart"> 
                <div class="lado frente">
                  <span>${element.location_area.name}<br> Max Cahnce (${
              element.version_details[0].max_chance
            })</span>
                </div> 
                <div class="lado atras">
                  ${
                    element.version_details
                      ? element.version_details.map(
                          (second) => `${second.version.name} <br>`
                        )
                      : ""
                  }
                </div>
      ...
      sprites: ({ sprites }) => {
        const sprite = Object.keys(sprites);
        const spriteArray = [];
        const listSprite = sprite.map((element, index) => {
          if (index < 8 && sprites[element] != null) {
            spriteArray.push(`<img alt="PokéAPI"src="${sprites[element]}"/>`);
          }
        });
        return `
